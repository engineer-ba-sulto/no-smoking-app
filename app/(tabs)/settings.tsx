...
import { ChevronRight, Bell, Cigarette, DollarSign, FileText, Shield } from 'lucide-react-native';
...
export default function SettingsScreen() {
      ...
      items: [
        {
          ...
          onPress: () => router.push('/edit-cigarettes'),
        },
        {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      >
        <Text className="text-2xl font-bold text-white text-center">せってい</Text>
      </LinearGradient>

      <ScrollView className="flex-1 px-5 pt-5" showsVerticalScrollIndicator={false}>
        {settingSections.map((section, sectionIndex) => (
          <View key={sectionIndex} className="mb-6">
            <Text className="text-base font-semibold text-gray-800 mb-3">▼ {section.title}</Text>
              ...
              {section.items.map((item, itemIndex) => (
                ...
                >
                  <View className="flex-row items-center flex-1">
                    {item.icon}
                    <Text className="text-sm font-medium text-gray-800 ml-3">{item.label}</Text>
                  </View>
                  <View className="flex-row items-center">
                    {item.toggle ? (
                      ...
                      <>
                        <Text className="text-sm text-gray-600 mr-2">{item.value}</Text>
